...
#       read preset list and connect
    ...
    def processStartup(self):
        ...
        for i in reversed(range(self.inner.layout().count())):
            self.inner.layout().itemAt(i).widget().setParent(None)
        programList = []
        self.buttonList = []
    ...
    def handleButtonStart(self):
        ...
        if choice == '0':
            ...
            brightnessVal = self.brightness.value()
            duration = self.duration.text()
            if duration == '':
                self.debugBrowser.append("No Duration entered. Default to 10 minutes")
                duration = 10
                self.duration.setText('10')
            brightnessStr = str(brightnessVal)
            durationStr = str(duration)

            if isRunning == True:
                self.debugBrowser.append("Stopping Program")
                temp = str(0)
                isRunning = False;
            ...
            string = choice+','+durationStr+','+brightnessStr+'\n'
            ...
            printResp = response.decode('ascii')
            print('\n'+printResp)
            response = arduino.readline()
            printResp = response.decode('ascii')
            print(printResp)
    ...
    def handleButtonStop(self):
        ...
        global isRunning
        if isRunning == False:
            self.debugBrowser.append("Already Stopped")
            return
        choice = str(0)
    ...
    def handleInteractive(self):
        ...
        while True:
            ...
            if choice == '0':
                ...
                arduino.write(ledDisplayInfo.encode()) #clear all lights
                time.sleep(.1)
                ledDisplayInfo = "-1,-1,-1\n"
                arduino.write(ledDisplayInfo.encode()) #tell arduino interactive is no longer selected
                break
